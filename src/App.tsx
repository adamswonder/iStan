...
const App: React.FC = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-gradient-to-b from-blue-50 to-gray-100">
      ...
      <section className="skeleton flex-1 p-4 space-y-6 w-full md:w-70vw lg:w-70vw mx-auto">
        <div className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-6 bg-blue-300 rounded-lg p-4">
          <img src={mainLogo} alt="Logo" className="w-24 md:w-32 object-contain" />
          <h3 className="text-blue-900 font-bold tracking-tight text-lg md:text-xl lg:text-2xl text-center md:text-left">EXPLORE SECURE BANKING SOLUTIONS</h3>
        </div>
        
        <div className="hero rounded-xl shadow-lg hover:shadow-xl transition-shadow h-60 sm:h-80 md:h-96 overflow-hidden">
          <img src={planningPng} alt="Hero" className="w-full h-full object-cover" />
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="content rounded-lg shadow-md hover:shadow-lg transition-shadow h-48 md:h-60">
            {/* <img src={deetsImage} alt="Feature 1" className="w-full h-full object-contain mb-4 rounded-lg" /> */}
          </div>
          <div className="content rounded-lg shadow-md hover:shadow-lg transition-shadow bg-white/80 p-4 h-48 md:h-60">
        ...
        <footer className="bg-blue-900 rounded-b-lg shadow-inner p-4 md:p-6 mt-8">
          <div className="flex items-center justify-between">
            <img src={mainLogo} alt="QR Code" className="w-24 md:w-30 h-8 md:h-10" />
            <img src={qrImage} alt="QR Code" className="w-24 md:w-30 h-8 md:h-10" />
          </div>
        </footer>
      </section>
    </div>
